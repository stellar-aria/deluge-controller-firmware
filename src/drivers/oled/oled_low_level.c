...
#include "platform/timers_interrupts/timers_interrupts.h"
...
// Returns true if the SPI bus is busy (any transfer) or an OLED (destinationId==0) entry is
// already in the queue, so the caller need not enqueue a duplicate OLED transfer.
bool spiHasPendingOledTransfer(void)
{
    if (spiTransferQueueCurrentlySending) {
        return true;
    }
    uint8_t pos = spiTransferQueueReadPos;
    while (pos != spiTransferQueueWritePos) {
        if (spiTransferQueue[pos].destinationId == 0) {
            return true;
        }
        pos = (pos + 1) & (SPI_TRANSFER_QUEUE_SIZE - 1);
    }
    return false;
}
