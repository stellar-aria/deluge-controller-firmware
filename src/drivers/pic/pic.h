...
public:
	...
	static void setColourForTwoColumns(size_t idx, const std::array<RGB, kDisplayHeight * 2>& colours) {
		send(std::to_underlying(Message::SET_COLOUR_FOR_TWO_COLUMNS) + idx);
	...
	static void setLEDOff(size_t idx) { send(std::to_underlying(Message::SET_LED_OFF) + idx); }
	static void setLEDOn(size_t idx) { send(std::to_underlying(Message::SET_LED_ON) + idx); }
	...
	static void flashMainPad(size_t idx) { send(std::to_underlying(Message::SET_PAD_FLASHING) + idx); }
	...
	static void flashMainPadWithColourIdx(size_t idx, int32_t colour_idx) {
		send(std::to_underlying(Message::SET_FLASH_COLOR) + colour_idx);
		flashMainPad(idx);
	}
	...
	static void sendScrollRow(size_t idx, RGB colour) {
		send(std::to_underlying(Message::SET_SCROLL_ROW) + idx);
		send(colour);
	}

	static void setupHorizontalScroll(uint8_t bitflags) {
		send(std::to_underlying(Message::SET_SCROLL_LEFT) + bitflags);
	}
...
private:
	 ...
	 */
	static void send(Message msg) { send(std::to_underlying(msg)); }
